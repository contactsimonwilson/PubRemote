...
static esp_err_t panel_co5300_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end,
...
esp_err_t esp_lcd_new_panel_co5300(const esp_lcd_panel_io_handle_t io,
  ...
  case 18: // RGB666
    ...
    // each color component (R/G/B) should occupy the 6 high bits of a byte, which means 3 full bytes are required for a
    // pixel
...
static esp_err_t tx_param(co5300_panel_t *co5300, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void *param,
                          size_t param_size) {
...
static esp_err_t tx_color(co5300_panel_t *co5300, esp_lcd_panel_io_handle_t io, int lcd_cmd, const void *param,
                          size_t param_size) {
  if (co5300->flags.use_qspi_interface) {
    lcd_cmd &= 0xff;
    lcd_cmd <<= 8;
...
static esp_err_t panel_co5300_init(esp_lcd_panel_t *panel) {
  ...
  ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_MADCTL,
                               (uint8_t[]){
                                   co5300->madctl_val,
                               },
                               1),
                      TAG, "send command failed");
  ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_COLMOD,
                               (uint8_t[]){
                                   co5300->colmod_val,
                               },
                               1),
...
static esp_err_t panel_co5300_draw_bitmap(esp_lcd_panel_t *panel, int x_start, int y_start, int x_end, int y_end,
  ...
  ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_CASET,
                               (uint8_t[]){
                                   (x_start >> 8) & 0xFF,
                                   x_start & 0xFF,
                                   ((x_end - 1) >> 8) & 0xFF,
                               ...
                               4),
                      TAG, "send command failed");
  ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_RASET,
                               (uint8_t[]){
                                   (y_start >> 8) & 0xFF,
                                   y_start & 0xFF,
                                   ((y_end - 1) >> 8) & 0xFF,
...
static esp_err_t panel_co5300_mirror(esp_lcd_panel_t *panel, bool mirror_x, bool mirror_y) {
  ...
  ESP_RETURN_ON_ERROR(tx_param(co5300, io, LCD_CMD_MADCTL, (uint8_t[]){co5300->madctl_val}, 1), TAG,
                      "send command failed");
  return ret;
}
