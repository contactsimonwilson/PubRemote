...
static void button_press_up_cb(void *arg, void *data) {
  ...
  ESP_LOGI(TAG, "BTN%d: BUTTON_PRESS_UP[%d]", get_btn_index((button_handle_t)arg),
           iot_button_get_ticks_time((button_handle_t)arg));
}

static void button_press_repeat_cb(void *arg, void *data) {
  ESP_LOGI(TAG, "BTN%d: BUTTON_PRESS_REPEAT[%d]", get_btn_index((button_handle_t)arg),
...
static void button_long_press_hold_cb(void *arg, void *data) {
  ...
  ESP_LOGI(TAG, "BTN%d: BUTTON_LONG_PRESS_HOLD[%d],count is [%d]", get_btn_index((button_handle_t)arg),
           iot_button_get_ticks_time((button_handle_t)arg), iot_button_get_long_press_hold_cnt((button_handle_t)arg));
}
...
static void button_press_repeat_done_cb(void *arg, void *data) {
  ...
  ESP_LOGI(TAG, "BTN%d: BUTTON_PRESS_REPEAT_DONE[%d]", get_btn_index((button_handle_t)arg),
           iot_button_get_repeat((button_handle_t)arg));
}
...
TEST_CASE("matrix keyboard button test", "[button][matrix key]") {
  ...
  button_config_t cfg = {.type = BUTTON_TYPE_MATRIX,
                         .long_press_time = CONFIG_BUTTON_LONG_PRESS_TIME_MS,
                         .short_press_time = CONFIG_BUTTON_SHORT_PRESS_TIME_MS,
...
TEST_CASE("adc button test", "[button][iot]") {
  ...
  for (size_t i = 0; i < 6; i++) {
    cfg.adc_button_config.adc_channel = 7, cfg.adc_button_config.button_index = i;
    if (i == 0) {
